    ...
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Draft entries remain editable; signed logs are locked.</p>
        </div>
      ...
      <div className="overflow-hidden rounded-md border bg-white shadow-sm">
        <table className="w-full text-left text-sm">
          <thead className="bg-muted">
            <tr><th className="p-3">Date</th><th className="p-3">Client</th><th className="p-3">Staff</th><th className="p-3">Status</th><th className="p-3">Report</th></tr>
          </thead>
          <tbody>
            {entries.map((entry) => (
              <tr className="border-t" key={entry.id}>
                ...
                <td className="p-3">{entry.status === "SIGNED" ? <Link className="text-primary underline" href={`/api/reports/${entry.id}`}>PDF</Link> : "Unavailable"}</td>
              </tr>
            ))}
          </tbody>
        </table>
